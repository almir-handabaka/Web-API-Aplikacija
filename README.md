...
Retrofit je type-safe HTTP klijent napisan za Android i Javu. Pomoću njega se povezujemo i dobijamo podatke sa nekog API-ja. Retrofit lagano parsira JSON i XML u Kotlin odgovarajuci kod. To se postiže korištenjem Retrofit (i Moshi) anotacije. 
#### Prikazivanje slika pomoću Glide biblioteke u grid view-u
Glide je biblioteka za učitavanje i keširanje slika u Android aplikacijama, Glide podržava puno vrsta fajlova, od png i jpg do gifova i video snimaka.
#### Recycler View
...
Kako bismo uzeli u obzir mrežne zahtjeve koji bi mogli potrajati dugo, mogli bismo razmotriti keširanje mrežnih odgovora u lokalnoj pohrani. To bi značilo da bi aplikacija mogla brzo prikazati keširani odgovor kada korisnik otvori aplikaciju, sve dok svježi podaci ne stignu sa servera.
Kako sloj podataka aplikacije postaje složeniji, najbolja praksa je apstrahovati ove detalje od UI koda. Obrazac (Repository) dizajna spremišta pruža čist API tako da ostatak aplikacije može lako dohvatiti podatke. Klasa spremišta zna odakle dobiti podatke i koje pozive API-ja treba izvršiti kada se podaci ažuriraju. Možete smatrati da su spremišta posrednici između različitih izvora podataka, kao što su trajni modeli, web servisi i keš memorije.
